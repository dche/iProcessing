...
- (void)setup
{
    [self size:320 :416];
    // Create buffered image for plasma effect
    pg = [self createGraphics:160 :90 :QUARTZ2D];
    [pg retain];
    [self colorMode:HSB];
    [self noSmooth];
}
...
- (void)draw
{
    ...
    // This runs plasma as fast as your computer can handle
    int timeDisplacement = [self frameCount];
    
    // No need to do this math for every pixel
    float calculation1 = [self sin:[self radians:timeDisplacement * 0.61655617f]];
    float calculation2 = [self sin:[self radians:timeDisplacement * -3.6352262f]];
    ...
    {
        float  yc    = 25;
        float s1 = 128 + 128 * [self sin:[self radians:xc] * calculation1];
        
        for (int y = 0; y < pg.height; y++, yc += pixelSize)
        {
            float s2 = 128 + 128 * [self sin:[self radians:yc] * calculation2];
            float s3 = 128 + 128 * [self sin:[self radians:(xc + yc + timeDisplacement * 5) / 2.0f]];  
            float s  = (s1+ s2 + s3) / 3;
            pg.pixels[(int)(x+y*pg.width)] = [self color:s :255 - s / 2.0f :255];
        }
    }   
    [pg updatePixels];
    [pg endDraw];
    ...
    // display the results
    [self image:[pg get] :0 :0 :self.width :self.height]; 
